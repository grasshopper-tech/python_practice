  ...
  def explore(self):
    ...
    while current_room is not "treasure room":
      ...
      for connected_room, weight in node.edges.items():
        ...
        print("enter {0} for {1}: {2} cost".format(str(key), str(connected_room), str(weight)))
      valid_choices = [room[:1] for room in node.edges.keys()]
      print("\nYou have accumulated: {0} cost".format(str(path_total)))
      choice = input("\nWhich room do you move to? ")
      if choice not in valid_choices:
        print("please select from these letters: {0}".format(str(valid_choices)))
      else:
    ...
    print("Made it to the treasure room with {0} cost".format(str(path_total)))
